...
class ResolvedConfig {
  ...
  async resolveErrorConfig(functionalTask) {
    const [schedule, maxNumFails, skip, continueOnFinalFail] = Promise.all([
      Resolve.toValue(functionalTask.canFail.schedule, Schedule),
      Resolve.optionalToValue(this._defaults.maxNumFails, functionalTask.canFail.maxNumFails, Number),
      Resolve.optionalToValue(this._defaults.skip, functionalTask.canFail.skip, Boolean),
      Resolve.optionalToValue(this._defaults.continueOnFinalFail, functionalTask.canFail.continueOnFinalFail, Boolean)
    ]);
  ...
  _createFunctionalTaskErrorFromDef(def = {}) {
    ...
    let obj = {
      ...
      maxNumFails: this._defaults.maxNumFails,
