...
, { Job, JobEvent, JobQueue, JobQueueCapabilities,
    ...
    symbolIdle, defer, Resolve
  } = require('sh.orchestration-tools')
, EventEmitter = require('events').EventEmitter
, symbolShutdown = Symbol('shutdown')
, { Observable, fromEvent } = require('rxjs')
, { LogLevel, BaseLogger, BaseScope, ColoredConsoleLogger, DevNullLogger } = require('sh.log-client');
...
class CameleerQueue {
  ...
  constructor(queueConfig, logger) {
    ...
    this.queue.observableRun.subscribe(next => {
      this.logger.logInfo(`Running Job #${next.job.id}.`);
    });
    this.queue.observableIdle.subscribe(next => {
      this.logger.logInfo('Queue is idle.');
    });
    this.queue.observableDone.subscribe(next => {
      this.logger.logInfo(`Job #${next.job.id} is done.`);
    });
    this.queue.observableFailed.subscribe(next => {
      this.logger.logError(`Job #${next.job.id} errored.`);
      this.logger.logDebug(next.error);
    });
  };
};
...
class Cameleer extends EventEmitter {
  ...
  constructor(configProvider) {
    ...
    /** @type {Observable.<void>} */
    this.observableShutdown = Object.freeze(fromEvent(this, symbolShutdown));
  };
  ...
  _initializeQueues(queueConf) {
    for (const conf of queueConf) {
      this._queues[conf.name] = new CameleerQueue(
        conf, this.getLogger(`${CameleerQueue.name} (${conf.name})`));
    }
    this.logger.logDebug(`Initialized ${queueConf.length} queue(s): ${this._queuesArr.map(q => `'${q.name}'`).join(', ')}`);
  };
  ...
  _initializeLogging(loggingConf) {
    ...
    this.logger.logDebug('Initialized new Cameleer instance.');
  ...
  async _enqueueTask(task) {
    ...
    this.logger.logDebug(`Config for task '${task.name}' has been resolved.`);
    ...
    if (!config.allowMultiple && this._isTaskRunning(task)) {
      this.logger.logInfo(`Task '${task.name}' is not allowed to run multiple times.`);
      return; // This task may only run once and there is an instance running already
    }
    if (config.queues.length === 0) {
      this.logger.logInfo(`No queues are defined, cannot run task '${task.name}'.`);
  ...
  clearTasks() {
    ...
    for (const task of this._tasksArr) {
      ...
      } else if (task.config.schedule instanceof ManualSchedule) {
        this._schedManual.removeManualSchedule(task.config.schedule);
      }
  ...
  async loadTasks() {
    ...
    for (let confOrTask of allConfigs) {
      if (!(confOrTask instanceof Task)) {
        confOrTask.enabled = await Resolve.toValue(confOrTask.enabled, Boolean);
        confOrTask = Task.fromConfiguration(confOrTask);
      }

      if (!(confOrTask instanceof Task)) {
        throw new Error(`The configuration does not resolve to or is not a Task: ${util.inspect(confOrTask)}`);
      }

      /** @type {Task} */
      const task = confOrTask;
      // For each run of the task, we'll enter a scope (and exit it as well)
      task.logger = this.getLogger(task.config.type);
      ...
      if (task.config.schedule instanceof Interval) {
        ...
        this._schedInterval.getObservableForSchedule(task.config.schedule).subscribe(enqueueTask);
      } else if (task.config.schedule instanceof Calendar) {
        await this._schedCalendar.addCalendar(task.config.schedule, false);
        this._schedCalendar.getObservableForSchedule(task.config.schedule).subscribe(enqueueTask);
      } else if (task.config.schedule instanceof ManualSchedule) {
        this._schedManual.addManualSchedule(task.config.schedule);
        this._schedManual.getObservableForSchedule(task.config.schedule).subscribe(enqueueTask);
      } else {
        throw new Error(`The given schedule for task '${task.name}' is not supported.`);
      }
    }
  ...
  runAsync() {
    ...
    const subs = this.observableShutdown.subscribe(next => {
      subs.unsubscribe();
    ...
    if (checkAllIdle()) {
      ...
      const subs = this._queuesArr.map(cq => cq.queue.observableIdle.subscribe(next => {
        if (checkAllIdle()) {
          ...
          this.logger.logInfo(`Pausing awaited, took ${(((+new Date) - now) / 1e3).toFixed(2)} seconds.`);
          deferred.resolve(this);
        }
      }));
    }
...
class CameleerJob extends Job {
  ...
  /**
   * Overridden to return the result of the last functional task. If not result
   * exists yet, will return undefined.
   * 
  ...
  async _attempt() {
    ...
    for (const funcTaskConf of this.conf.tasks) {
      ...
      try {
        this.task.logger.logInfo(`Attempting Job #${this.id} (functional task #${fTaskIdx}${funcTaskConf.name === void 0 ? '' : ` ('${funcTaskConf.name}')`}) of task '${this.task.name}'..`);
        ...
        this.task.logger.endScope(logScope);
