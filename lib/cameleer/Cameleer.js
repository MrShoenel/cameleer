...
class Cameleer extends EventEmitter {
  ...
  async _enqueueTask(task, schedEvent) {
    ...
    const job = new CameleerJob(task, config, schedEvent, this);
    /** @type {CameleerQueue} */
    let queue = null;
    try {
    ...
    race(queue.queue.observableDone, queue.queue.observableFailed).subscribe(function(jqEvt) {
      if (jqEvt.job === job) {
        ...
        if (hasFailed) {
          that.logger.logError(`The Job has failed after ${formatDuration(durationSecs)}.`);
        } else {
          that.logger.logInfo(`The job succeeded after ${formatDuration(durationSecs)}.`);
        }
...
class CameleerJob extends Job {
   ...
   */
  async _attempt() {
    const nameScope = this.task.logger.beginScope(this.task.name);
    ...
    try {
      ...
      for (const funcTaskConf of this.conf.tasks) {
        ...
        try {
          ...
          this.task.logger.logDebug(`Attempting Job #${this.id} (functional task #${fTaskName} of task '${this.task.name}')..`);
          ...
          this.task.logger.logError(`The error was: ${wrappedErrMsg}`, attemptErr.wrappedErr);
      ...
      this.task.logger.endScope(nameScope);
    }
  };
};
