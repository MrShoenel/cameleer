...
class Cameleer extends EventEmitter {
  ...
  _selectBestMatchingQueue(config) {
    const defaultQueues = this._queuesArr.filter(q => q.isDefault)
    , defaultCostQueue = defaultQueues.find(cq => !cq.isParallel) || null
    , defaultParallelQueue = defaultQueues.find(cq => cq.isParallel) || null
    , defaultQueue = config.cost === null ? defaultParallelQueue : defaultCostQueue
    , hasDefaultQueue = defaultQueue instanceof CameleerQueue;
    ...
    if (cameleerQueues.length === 0) { // Task wants to use queues we don't have:
    ...
    // Let's try to use a default queue before falling back to the least-busy:
    if (hasDefaultQueue) {
      return defaultQueue;
    }

    // Here we select the least busy (or: smallest backlog/cost/load).
