...
class Cameleer extends EventEmitter {
  ...
  _selectBestMatchingQueue(config) {
    const cameleerQueues = config.queues
      .filter(qName => this._queues.hasOwnProperty(qName))
  ...
  async _enqueueTask(task) {
    ...
    if (config.queues.length === 0) {
      this.logger.logInfo(`The task '${task.name}' did not specify any queues it is allowed to run on.`);
...
class JobFailError extends Error {
  ...
  constructor(previousError = void 0) {
    super(previousError instanceof Error ? previousError.message : void 0);
