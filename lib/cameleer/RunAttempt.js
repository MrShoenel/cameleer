...
class RunAttempt {
  ...
  constructor(conf, job) {
    ...
    this.typeName = typeof job.conf.type === 'function' ? job.conf.type.name : job.conf.type;

    if (job.cameleer !== null) {
      /** @type {BaseLogger.<*>} */
      this.logger = job.cameleer.getLogger(RunAttempt);
      this._logScopeState = `${typeof job.conf.type === 'function' ? job.conf.type.name : job.conf.type}, ${this.fTaskName}`;
    } else {
      this.logger = null;
    }
  ...
  _logDebug(message) {
    ...
    this.logger.withScope(this.typeName, (sc, l) => {
      l.withScope(`#${this.job.id}`, (sc1, l1) => {
        l1.withScope(this.fTaskName, (sc2, l2) => {
          l2.logDebug(message);
        });
      });
    });
