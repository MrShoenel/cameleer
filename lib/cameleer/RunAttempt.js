...
class RunAttempt {
   ...
   */
  get previousResult() {
    return this.task.results.length === 0 ? void 0 : this.task.results.reverse()[0];
  };
  ...
  async run() {
    ...
    try {
      return Result.fromValue.fromValue(await wrapFunc());
    } catch (err) {
  ...
  async _runErrored(err, wrapFunc) {
    ...
    try {
      ...
      if (skip) {
        ...
        return Result.fromError.fromError(err);
      }
