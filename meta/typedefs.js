 ...
 * @property {(rro: ResolvedResolveObject) => (boolean|Promise.<boolean>} [skip] Optional. Defaults to false. A function that returns a boolean (or a Promise that resolves to a boolean) value to indicate whether or not this task should be skipped at the time of evaluation. It is evaluated before any other tasks are run. If a non-boolean value is returned or the Promise is rejected, the task will be aborted. This property is evaluated every time the task is scheduled to run.
 * @property {number|((rro: ResolvedResolveObject) => (number|Promise.<number>)} [cost] Optional. Defaults to null. If this task is allowed to run on cost-based queues, it needs to define a cost according to the queue's capabilities. This property is evaluated every time the task is scheduled to run.
 * @property {boolean|((rro: ResolvedResolveObject) => (boolean|Promise.<boolean>)} [allowMultiple] Optional. Defaults to false. If true, multiple instances of this task may run in parallel, if scheduled. If false, scheduling attempts will be ignored while the task is running. This property is evaluated every time the task is scheduled to run.
 * @property {Array.<string>|((rro: ResolvedResolveObject) => (Array.<string>|Promise.<Array.<string>>)} [queues] Optional. Defaults to []. An array of names of queues, this task is allowed to run on. Queues are checked in the order they appear and the first matching queue that has a free slot (in case of parallel queues) or enough capabilities is selected to run the job. If the task does not define queues to run on and compatible default-queues are defined, they will be selected. If no appropriate queue is found, the job is enqueued in the least busy queue. If the task defines queues to run on and none of these is available to Cameleer, an error is thrown. If this property does not return an array of strings or e.g. the promise is rejected, then the job is not run and aborted. This property is evaluated every time the task is scheduled to run.
 * @property {Progress|((rro: ResolvedResolveObject) => (Progress|Promise.<Progress>))} [progress] Optional. Defaults to null. A Progress-object that will be observed for progress, while this task is running. This property is evaluated every time the task is scheduled to run.
 * @property {Schedule|((rro: ResolvedResolveObject) => (Schedule|Promise.<Schedule>))} schedule The schedule this job uses to schedule when it should be triggered. This schedule will internally be added to an appropriate scheduler. Note that this property is only evaluated once during task creation (i.e. the schedule of a task cannot be changed later).
